...
results_table <- data.frame(
  Metric = c("Correlation Coefficient", "P-Value", "T Statistic", "Degrees of Freedom", "95% Confidence Interval (Lower)", "95% Confidence Interval (Upper)"),
  Value = c(
...
# Print a beautiful table with kableExtra
results_table %>%
  kbl(caption = "Results of Pearson Correlation Test between Bechdel Test Results and Revenue") %>%
...
1.  **Budget**: Strong and significant positive effect (p\<0.001), showing that higher budgets are associated with more nominations.

2.  **Bechdel Test**: Not significant (p=0.63), suggesting no direct influence on nominations.
...
We created a model to predict in advance the award winning potential of a movie. This model aims to guide studios and producers, marketing agencies in their investments in films. You can find it in the following link: [Movie Awards Predictor](https://684o3y-dorra.shinyapps.io/bechdel-app/)
...
train_index <- sample(1:nrow(df_gender), 0.7 * nrow(df_gender))  # 70% for training
train_data <- df_gender[train_index, ]
test_data <- df_gender[-train_index, ]
...
rf_model_Awards <- randomForest(Awards ~ Bechdel_binary + Budget + Imdb_Rating + Gender, 
                         data = train_data, 
                         importance = TRUE)

# Extract variable importance
rf_summary <- as.data.frame(importance(rf_model_Awards)) %>%
...
rf_predictions <- predict(rf_model_Awards, test_data)
...
-   The model performance of the random forest for awards has an **accuracy of** 78% but the **sensitivity** is at 29%, meaning the model struggles to identify films that win awards. This can be explain by the asymmetry between the number of movies that have received awards (79.26%) and the one that didn't (20.21%).
...
-   The Random Forest model demonstrates an accuracy of **61%**, indicating that it correctly predicts the Bechdel binary outcome in 61% of cases. The sensitivity is **72%**, meaning it effectively identifies positive cases, while the specificity is **46%**, reflecting a lower ability to correctly classify negative cases, meaning movies that do not pass the test.
