...
datatable(df_final_gender, 
          options = list(pageLength = 10,  # Número de filas que se muestran por página
                         autoWidth = TRUE, # Ajuste automático del ancho de las columnas
                         searchHighlight = TRUE), # Resaltar texto de búsqueda
          filter = 'top')  # Agrega un filtro de búsqueda en la parte superior de la tabla
...
2.  **Bechdel Test Result (Passed/Failed)**\
...
9.  **Genre (Action, Drama, Comedy, etc.)**\
...
plot <- ggplot(
  ...
  theme(
    ...
    legend.position = c(1, 0.5),  # Center legend vertically on the right side
...
Statistical testing further supports the significance of this revenue difference, confirming that it is highly unlikely to have occurred by random chance. The t-test's low p-value (1.337e-11) indicates that this gap is consistent enough across the sample to suggest a systematic pattern rather than an isolated anomaly. The implication here is that gender-based revenue differences are reflective of broader trends in the film industry, not just singular outliers.
